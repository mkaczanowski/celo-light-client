...
use parity_scale_codec::{Encode as EncodeTrait, Decode as DecodeTrait, Error, Input, Output};
...
impl DB {
    ...
    pub fn has_key(&self, col: u32, key: &[u8]) -> io::Result<bool> {
        KeyValueDB::get(self, col, key).map(|opt| opt.is_some())
    }
...
// Celo specific traits
impl<T: EncodeTrait + DecodeTrait> SerializableStorage<T> for DB {
    fn serialize(&self) -> Vec<u8> {
        self.encode()
    }

    fn deserialize(bytes: &[u8]) -> Result<Box<T>, crate::Error> {
       let mut bytes = bytes.clone();
       let new_db: T = match T::decode(&mut bytes) {
           Ok(db) => db,
           Err(e) => return Err(Kind::GenericSerializationError.context(e).into())
       };
       Ok(Box::new(new_db))
    }
}
...
impl Storage for DB {
    fn put(&mut self, key: &[u8], value: &[u8]) -> Result<Option<Vec<u8>>, crate::Error> {
        let mut transaction = self.transaction();
        transaction.put(0, key, value);

        match self.write(transaction) {
            Ok(_) => Ok(None),
            Err(e) => Err(Kind::DbError.context(e).into()),
        }
    }

    fn get(&self, key: &[u8]) -> Result<Vec<u8>, crate::Error> {
        match KeyValueDB::get(self, 0, key) {
    ...
    fn contains_key(&self, key: &[u8]) -> Result<bool, crate::Error> {
        match self.has_key(0, key) {
            Ok(val) => Ok(val),
            Err(e) => Err(Kind::DbError.context(e).into())
        }
    }
...
mod tests {
    ...
    use kvdb::KeyValueDB;
    use parity_scale_codec::{Decode as DecodeTrait, Encode as EncodeTrait};
